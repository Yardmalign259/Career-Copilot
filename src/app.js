...
function saveApiKey() {
  ...
  if (!key.startsWith("gsk_")) { alert('Valid Groq API key "gsk_" se shuru hoti hai.'); return; }
  state.apiKey = key;
  setApiKey(key);
  apiModal.hide();
...
async function handleAnalyzeResume() {
  ...
  if (!text || text.length < 80) { alert("Resume upload karo ya paste karo pehle!"); return; }
  ...
  } catch (e) { stop(); alert("Error: " + e.message); }
...
async function handleGenerateQuestion(forceNew = false) {
  ...
  try {
    const raw = await generateInterviewQuestion({ role, type, forceNew, count: ++state.questionCount, apiKey: state.apiKey });
...
async function handleGetFeedback() {
  ...
  try {
    const result = await evaluateAnswer({ question: state.currentQuestion, answer, role, type, apiKey: state.apiKey });
    stop();
    addChatMsg("chat-container", "ai", parseMarkdown(result));
    const session = { date: new Date().toISOString(), role, type, question: state.currentQuestion, answer, feedback: parseMarkdown(result), score: "N/A" };
    saveSession(session);
    renderHistory();
  } catch (e) { stop(); alert("Error: " + e.message); }
...
async function handleGetTips() {
  ...
  $("chat-container").innerHTML = "";
  try {
    const result = await getAnswerTips({ question: state.currentQuestion, role, type, apiKey: state.apiKey });
    stop();
    addChatMsg("chat-container", "ai", parseMarkdown(result));
  } catch (e) { stop(); alert("Error: " + e.message); }
...
function renderHistory() {
  ...
  if (history.length === 0) { container.innerHTML = "<p>No sessions yet.</p>"; return; }
  container.innerHTML = history.map((s) => `
    <div class="history-item" style="border:1px solid var(--color-border);padding:var(--space-3);margin-bottom:var(--space-3);cursor:pointer;border-radius:var(--radius);">
      <div><strong>${new Date(s.date).toLocaleString()}</strong> — ${s.role}</div>
      <div style="display:none;" class="history-details" style="margin-top:var(--space-2);">
